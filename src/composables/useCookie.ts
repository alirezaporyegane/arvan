...
class VueCookiesManager implements Cookies {
      ...
      switch (expireTimes.constructor) {
        ...
        case String:
          if (/^(?:\d+(y|m|d|h|min|s))$/i.test(<string>expireTimes)) {
            ...
            const _expireTime = (<string>expireTimes).replace(/^(\d+)(?:y|m|d|h|min|s)$/i, '$1')
            // get capture type group , to lower case
            switch (
              (<string>expireTimes).replace(/^(?:\d+)(y|m|d|h|min|s)$/i, '$1')?.toLowerCase()
            ) {
              // Frequency sorting
              case 'm':
  ...
  isKey(keyName: string): boolean {
    return new RegExp(
      '(?:^|;\\s*)' + encodeURIComponent(keyName).replace(/[\-\.\+\*]/g, '\\$&') + '\\s*\\='
